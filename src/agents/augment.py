...
class AugmentAgent(Agent):
    ...
    def run(self, input: AgentInput):
        ...
        if agent_input.aprompt is not None:
            prompt_template = agent_input.aprompt
        else:
        ...
        sample_row = agent_input.df.sample(1).to_dict(orient="records")[0]

        # Prepare data
        summary_dict = summarize_dataframe(agent_input.df).reset_index()
        ...
        augmentation_section = (
            f"=== AUGMENTATION GOAL ===\n{agent_input.augmentation_goal}"
            if agent_input.augmentation_goal
            else ""
        )
        ...
        prompt = prompt_template.format(
            primary_domain=agent_input.domain_context.get("primary_domain", "Unknown"),
            column_descriptions=json.dumps(
                agent_input.domain_context.get("column_descriptions", {}), indent=2
            ),
            ...
            sparql_result="none",
            column="column",
            num_columns_to_add=agent_input.num_columns_to_add,
        ...
        try:
            augmented_df = agent_input.df.copy()
            added_columns = []
            for suggestion in suggestions:
                col_name = suggestion["name"]
                # Exclude columns that use the target column in their generation method
                if agent_input.target_column and agent_input.target_column in suggestion.get(
                    "generation_method", ""
                ):
                    print(
                        f"Skipping column '{col_name}' because it uses the target column '{agent_input.target_column}'."
                    )
                    continue
